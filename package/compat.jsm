...
const EPYRUS_NAME = "Epyrus";
const SEAMONKEY_NAME = "SeaMonkey";
...
function isEpyrus() {
  if (gIsEpyrus === null) {
    gIsEpyrus = Cc["@mozilla.org/xre/app-info;1"].getService(Ci.nsIXULAppInfo).name === EPYRUS_NAME;
  }
  return gIsEpyrus;
}

function isSeaMonkey() {
  if (gIsSeaMonkey === null) {
    gIsSeaMonkey = Cc["@mozilla.org/xre/app-info;1"].getService(Ci.nsIXULAppInfo).name === SEAMONKEY_NAME;
  }
  return gIsSeaMonkey;
