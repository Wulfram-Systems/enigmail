...
var EnigmailCore = {
  ...
  /**
   * Check if Enigmail is initialized. Unlike getService, this doesn't try 
   * to initialize Enigmail
   * 
   * @return {Boolean}: true  - Enigmail is completely initialized
   *                    false - Enigmail is not initialized or initialization is ongoing
   */ 
  isEnigmailInitialized: function() {
    if ((!gEnigmailService) || (!gEnigmailService.initialized)) {      
      return false;
    }
...
Enigmail.prototype = {
  ...
  initialize: function(domWindow, version) {
    ...
    getEnigmailLog().DEBUG(`core.jsm: starting key refreshing service\n`);
    getEnigmailKeyRefreshService().start(getEnigmailKeyServer());

    getEnigmailLog().DEBUG(`core.jsm: attempting post-init functions\n`);
    ...
    for (let fn of this._postInitFunctions) {
      try {
        ...
        getEnigmailLog().DEBUG(`core.jsm: Error with post-init function ${fn}:\n${x.toString()}\n`);
      }
    }
